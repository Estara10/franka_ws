...
class _MirroredLogger:
    """将 ROS logger 输出同时写入本地文件。"""
    def __init__(self, base_logger, sink_callback):
        self._base_logger = base_logger
        self._sink_callback = sink_callback

    def debug(self, msg, *args, **kwargs):
        self._sink_callback('DEBUG', msg)
        return self._base_logger.debug(msg, *args, **kwargs)

    def info(self, msg, *args, **kwargs):
        self._sink_callback('INFO', msg)
        return self._base_logger.info(msg, *args, **kwargs)

    def warn(self, msg, *args, **kwargs):
        self._sink_callback('WARN', msg)
        return self._base_logger.warn(msg, *args, **kwargs)
    ...
    def error(self, msg, *args, **kwargs):
        self._sink_callback('ERROR', msg)
        return self._base_logger.error(msg, *args, **kwargs)

    def fatal(self, msg, *args, **kwargs):
        self._sink_callback('FATAL', msg)
        return self._base_logger.fatal(msg, *args, **kwargs)
...
class DualArmTaskNode(Node, GripperOpsMixin, ServoOpsMixin, StateOpsMixin):
    def get_logger(self):
        return getattr(self, '_mirrored_logger', super().get_logger())

    def __init__(self, mode: str = 'auto'):
        super().__init__('dual_arm_task_node')
        self._base_logger = super().get_logger()
        self._mirrored_logger = self._base_logger
        self._runtime_log_lock = threading.Lock()
        self._runtime_log_fp = None
        self._runtime_log_path = ''
        self._runtime_log_io_failed = False
        self._setup_runtime_log_mirror()
        ...
        # State6 任务模式:
        #   - 'translate'         : 传统平移搬运 (TRANSPORT_X/Y/Z_OFFSET)
        #   - 'rotate_center_90'  : 围绕“原始抓取中心”旋转铝条 90°
        ...
        self.dual_traj_start_blend_vel = 0.06      # rad/s, 首点引入段速度参考
    ...
    def _setup_runtime_log_mirror(self):
        env_path = os.environ.get('DUAL_ARM_RUNTIME_LOG_FILE', '').strip()
        if env_path:
            log_path = Path(env_path).expanduser()
        else:
            workspace_root = Path(__file__).resolve().parents[3]
            log_path = workspace_root / 'log' / 'dual_arm_transport_runtime.log'
        ...
        try:
            ...
            self._runtime_log_path = str(log_path)
            self._mirrored_logger = self._base_logger
            self._base_logger.warn(f"日志镜像初始化失败({e})，将只输出到 ROS 日志通道")

    def _append_runtime_log_line(self, level: str, msg):
        fp = getattr(self, '_runtime_log_fp', None)
        if fp is None:
            return
        text = str(msg)
        ...
        try:
            ...
            if not self._runtime_log_io_failed:
                self._runtime_log_io_failed = True
                self._base_logger.warn(f"写入日志镜像文件失败({e})，后续仅输出到 ROS 日志通道")
    ...
    def close_runtime_log_file(self):
        ...
        if fp is None:
            return
        try:
            self._append_runtime_log_line('INFO', "==== 任务进程结束 ====")
        except Exception:
            pass
        try:
            with self._runtime_log_lock:
    ...
    def _wait_for_service_with_timeout(self, client, service_name: str, total_timeout: float = None) -> bool:
        """统一服务等待逻辑，避免在服务缺失时无限阻塞。"""
        if client is None:
            self.get_logger().error(f"服务客户端不存在: {service_name}")
            return False

        if total_timeout is None:
            total_timeout = float(getattr(self, 'service_wait_total_sec', 8.0))
        total_timeout = max(0.5, float(total_timeout))
        poll = min(1.0, max(0.2, float(getattr(self, 'action_wait_poll_sec', 1.0))))
        max_rounds = max(1, int(total_timeout / poll + 0.999))

        for round_idx in range(1, max_rounds + 1):
            remaining = max(0.0, total_timeout - (round_idx - 1) * poll)
            wait_slice = min(poll, remaining)
            if client.wait_for_service(timeout_sec=wait_slice):
                return True
            if round_idx == 1 or round_idx == max_rounds or round_idx % 3 == 0:
                self.get_logger().warn(
                    f"等待服务 {service_name} 中 ({round_idx}/{max_rounds})...")

        self.get_logger().error(f"服务等待超时: {service_name} (>{total_timeout:.1f}s)")
        return False

    async def compute_fk_async(self, fk_link_names, joint_names, joint_positions):
        if not self._wait_for_service_with_timeout(self.fk_client, '/compute_fk'):
            return None
        ...
        try:
            ...
            result = await asyncio.wait_for(
                future, timeout=max(2.0, float(getattr(self, 'service_wait_total_sec', 8.0)))
            )
        except asyncio.TimeoutError:
            self.get_logger().warn("compute_fk 服务调用超时")
            return None
        except Exception as e:
            self.get_logger().warn(f"compute_fk 服务调用异常: {e}")
            return None
    ...
    async def compute_ik_async(self, group_name: str, pose: PoseStamped, avoid_collisions: bool = True,
        ...
        """
        if not self._wait_for_service_with_timeout(self.ik_client, '/compute_ik'):
            return None
        ...
        try:
            ...
            result = await asyncio.wait_for(
                future, timeout=max(3.0, float(getattr(self, 'service_wait_total_sec', 8.0)))
            )
        except asyncio.TimeoutError:
            self.get_logger().warn(
                f"compute_ik 调用超时(group={group_name}, avoid_collisions={avoid_collisions})")
            return None
        except Exception as e:
            self.get_logger().warn(
                f"compute_ik 调用异常(group={group_name}, avoid_collisions={avoid_collisions}): {e}")
            return None
    ...
    def _is_rotate_transport_mode(self) -> bool:
        mode = str(getattr(self, 'transport_task_mode', 'translate')).strip().lower()
        return mode in ('rotate_center_90', 'rotate90', 'rotate_center', 'rotate')

    def _resolve_rotation_center_xy(self, left_pos, right_pos):
        # 优先使用抓取阶段的动态抓取点中心（“原来的中心”）
        p_left = getattr(self, 'dynamic_p_left', None)
        p_right = getattr(self, 'dynamic_p_right', None)
        if p_left is not None and p_right is not None and len(p_left) >= 2 and len(p_right) >= 2:
            center = np.array(
                [
                    0.5 * (float(p_left[0]) + float(p_right[0])),
                    0.5 * (float(p_left[1]) + float(p_right[1])),
                ],
                dtype=float,
            )
            return center, 'dynamic_grasp_center'

        # 其次使用任务起始时缓存的目标中心
        latest = getattr(self, 'latest_target_pose', None)
        if latest is not None:
            center = np.array(
                [float(latest.pose.position.x), float(latest.pose.position.y)],
                dtype=float,
            )
            return center, 'latest_target_pose'

        # 最后回退到当前双臂末端中心
        center = np.array(
            [
                0.5 * (float(left_pos[0]) + float(right_pos[0])),
                0.5 * (float(left_pos[1]) + float(right_pos[1])),
            ],
            dtype=float,
        )
        return center, 'current_ee_center'

    def _build_target_pose(self, pos_xyz, quat_xyzw):
        ps = PoseStamped()
        ps.header.frame_id = self.planning_frame
        ps.pose.position.x = float(pos_xyz[0])
        ps.pose.position.y = float(pos_xyz[1])
        ps.pose.position.z = float(pos_xyz[2])
    ...
    async def _execute_rotation_step_about_center(self, center_xy: np.ndarray,
                                                  step_deg: float,
                                                  pos_tol: float,
                                                  ori_tol: float,
                                                  rotate_orientation: bool) -> bool:
        left_now, left_q = await self.get_current_pose('mj_left_link8')
        right_now, right_q = await self.get_current_pose('mj_right_link8')
        if left_now is None or right_now is None or left_q is None or right_q is None:
            self.get_logger().warn("旋转步进失败：无法读取当前双臂末端位姿")
            return False

        step_rad = np.deg2rad(float(step_deg))
        rot_step = R.from_euler('z', step_rad)

        left_vec = np.array(
            [float(left_now[0]) - center_xy[0], float(left_now[1]) - center_xy[1], 0.0],
            dtype=float,
        )
        right_vec = np.array(
            [float(right_now[0]) - center_xy[0], float(right_now[1]) - center_xy[1], 0.0],
            dtype=float,
        )
        left_vec_rot = rot_step.apply(left_vec)
        right_vec_rot = rot_step.apply(right_vec)

        left_target_pos = np.array(
            [center_xy[0] + left_vec_rot[0], center_xy[1] + left_vec_rot[1], float(left_now[2])],
            dtype=float,
        )
        right_target_pos = np.array(
            [center_xy[0] + right_vec_rot[0], center_xy[1] + right_vec_rot[1], float(right_now[2])],
            dtype=float,
        )

        if rotate_orientation:
            left_target_q = (rot_step * R.from_quat(left_q)).as_quat()
            right_target_q = (rot_step * R.from_quat(right_q)).as_quat()
        else:
            left_target_q = np.array(left_q, dtype=float)
            right_target_q = np.array(right_q, dtype=float)

        strict_left = self._build_target_pose(left_target_pos, left_target_q)
        strict_right = self._build_target_pose(right_target_pos, right_target_q)
        candidate_goals = [('姿态跟随', strict_left, strict_right)]
        if rotate_orientation and bool(getattr(self, 'transport_rotate_try_without_orientation', True)):
            relaxed_left = self._build_target_pose(left_target_pos, left_q)
            relaxed_right = self._build_target_pose(right_target_pos, right_q)
            candidate_goals.append(('位置优先', relaxed_left, relaxed_right))

        async def _try_cartesian(left_goal_ps, right_goal_ps, desc: str) -> bool:
            old_frac = float(getattr(self, 'cartesian_fraction_threshold', 0.98))
            old_one_shot = float(getattr(self, 'cartesian_one_shot_fraction_threshold', old_frac))
            rotate_frac = max(
                0.90,
                min(
                    0.999,
                    float(getattr(self, 'transport_rotate_cartesian_fraction_threshold', 0.94))
                ),
            )
            self.cartesian_fraction_threshold = rotate_frac
            self.cartesian_one_shot_fraction_threshold = max(rotate_frac, old_one_shot)
            try:
                return await self._move_dual_cartesian_to_targets(
                    left_goal=left_goal_ps,
                    right_goal=right_goal_ps,
                    description=desc,
                ...
                self.cartesian_fraction_threshold = old_frac
                self.cartesian_one_shot_fraction_threshold = old_one_shot

        # 优先用笛卡尔路径微步进，避免 IK 先验失败导致整个旋转卡死
        if bool(getattr(self, 'transport_rotate_cartesian_first', True)):
            for label, left_goal, right_goal in candidate_goals:
                cart_ok = await _try_cartesian(
                    left_goal,
                    right_goal,
                    desc=f"State6旋转步进({label}笛卡尔 {step_deg:.2f}deg)",
                )
                if cart_ok:
                    return True
                self.get_logger().warn(f"旋转{label}笛卡尔步进失败，继续回退规划")

        async def _try_sequential(left_goal_ps, right_goal_ps, label: str) -> bool:
            left_ok = await self.move_arm_to_pose_async(
                'left',
                left_goal_ps,
                pos_tol=pos_tol,
                ori_tol_x=ori_tol,
                ori_tol_y=ori_tol,
                ori_tol_z=max(ori_tol, 0.18),
                max_retries=2,
            )
            if not left_ok:
                self.get_logger().warn(f"旋转顺序步进失败({label}): 左臂未到目标")
                return False

            pause_sec = max(0.0, float(getattr(self, 'transport_rotate_sequential_pause_sec', 0.02)))
            if pause_sec > 1e-3:
            ...
            if not right_ok:
                self.get_logger().warn(f"旋转顺序步进失败({label}): 右臂未到目标")
                return False
            return True

        # 可选顺序微步：仅作为笛卡尔失败后的回退，不再抢占主路径
        if bool(getattr(self, 'transport_rotate_sequential_mode', False)):
            for label, left_goal, right_goal in candidate_goals:
                if await _try_sequential(left_goal, right_goal, label):
                    return True

        # 最后回退到同步规划（可选临时开启 IK 碰撞回退）
        old_collision_fallback = bool(getattr(self, 'sync_plan_allow_collision_ik_fallback', False))
        if bool(getattr(self, 'transport_rotate_ik_collision_fallback', True)):
            self.sync_plan_allow_collision_ik_fallback = True
        try:
            for label, left_goal, right_goal in candidate_goals:
                ok = await self.sync_move_arms(
                    left_goal,
                    right_goal,
                    pos_tol=pos_tol,
                    ori_tol_x=ori_tol,
                ...
                if ok:
                    return True
                self.get_logger().warn(f"旋转同步规划失败({label})")
            return False
        finally:
            self.sync_plan_allow_collision_ik_fallback = old_collision_fallback

    async def state_execute_with_compliance(self):
        """State 6: 执行搬运。旋转模式下围绕原中心旋转铝条90°。"""
        if not self._is_rotate_transport_mode():
            return await StateOpsMixin.state_execute_with_compliance(self)
        ...
        if delta_z > 0.008:
            self.get_logger().info(
                f"开始搬运补偿抬升(旋转任务): current_z={current_ee_z}, "
                f"target={target_transport_z:.3f}, delta={delta_z:.3f}")
            ok_lift = await self._move_dual_cartesian_delta(
                delta_z=delta_z,
                description="State6补偿抬升(旋转任务)",
                max_step=0.012,
                avoid_collisions=False,
            ...
            if not ok_lift:
                self.get_logger().warn("State6 旋转任务补偿抬升失败，继续尝试旋转")
        else:
            self.get_logger().info("当前高度已足够，跳过 State 6 旋转任务额外抬升")
        ...
        if left_before is None or right_before is None:
            self.get_logger().error("State6 旋转任务无法获取当前位姿")
            self.current_state = TaskState.ERROR
            return

        center_xy, center_source = self._resolve_rotation_center_xy(left_before, right_before)
        total_deg = abs(float(getattr(self, 'transport_rotate_deg', 90.0)))
        direction = str(getattr(self, 'transport_rotate_direction', 'ccw')).strip().lower()
        if direction in ('cw', 'clockwise', 'right', '-'):
            total_deg = -total_deg
        if abs(total_deg) < 0.5:
            self.get_logger().warn("旋转角度过小，跳过旋转搬运")
            await asyncio.sleep(max(0.0, float(getattr(self, 'state_transition_pause_sec', 0.08))))
            self.current_state = TaskState.OPEN_GRIPPERS
            return

        step_limit = max(2.0, float(getattr(self, 'transport_rotate_step_deg', 10.0)))
        step_count = max(1, int(np.ceil(abs(total_deg) / step_limit)))
        step_deg = total_deg / float(step_count)
        pos_tol = max(0.006, float(getattr(self, 'transport_rotate_pos_tol', 0.015)))
        ori_tol = max(0.05, float(getattr(self, 'transport_rotate_ori_tol', 0.14)))
        rotate_orientation = bool(getattr(self, 'transport_rotate_with_orientation', True))
        settle_sec = max(0.0, float(getattr(self, 'transport_rotate_settle_sec', 0.02)))
        ...
        for idx in range(step_count):
            ...
            if not ok_step and abs(step_deg) > 2.5:
                self.get_logger().warn(
                    f"旋转步进[{idx + 1}/{step_count}]失败，尝试二分步重试")
                half = 0.5 * step_deg
                ok_step = await self._execute_rotation_step_about_center(
                    center_xy=center_xy,
                    step_deg=half,
            ...
            if not ok_step:
                msg = f"State6 旋转步进失败(step={idx + 1}/{step_count})"
                if self._is_force_continue_enabled():
                    self.get_logger().warn(f"{msg}，demo模式继续后续流程")
                    break
    ...
    async def _execute_merged_trajectory(self, left_traj, right_traj, time_scale=1.3):
        ...
        left_points = list(getattr(left_traj, 'points', []) or [])
        right_points = list(getattr(right_traj, 'points', []) or [])
        if not left_points or not right_points:
            self.get_logger().warn(
                f"合并轨迹失败：空轨迹输入(left_points={len(left_points)}, right_points={len(right_points)})")
            return False
        ...
        for pt in left_points:
            t_set.add(max(0.0, float(get_dur_sec(pt))))
        ...
        def interp(points, target_t):
            target_t = target_t / time_scale
            if target_t <= 0.0:
                return list(points[0].positions)
            last_t = get_dur_sec(points[-1])
            if target_t >= last_t:
                return list(points[-1].positions)
            for i in range(len(points) - 1):
                t1, t2 = get_dur_sec(points[i]), get_dur_sec(points[i + 1])
                if t1 <= target_t <= t2:
                    r = (target_t - t1) / max(t2 - t1, 1e-6)
                    return [p1 + r * (p2 - p1) for p1, p2 in zip(points[i].positions, points[i + 1].positions)]
            return list(points[-1].positions)
        ...
        for t in t_unified:
            ...
            pt.positions = interp(left_points, t) + interp(right_points, t)
            set_dur(pt, t)
        ...
        # 时间轴加密：当点间隔过大时重采样，避免“长时间只给少量航点”引发的水平段卡顿
        if len(merged.points) >= 2:
            max_gap = max(
                t_unified[idx] - t_unified[idx - 1]
                for idx in range(1, len(t_unified))
            )
            resample_dt_base = max(min_dt, float(getattr(self, 'dual_traj_resample_dt', 0.08)))
            resample_gap_trigger = max(
                resample_dt_base * 1.1,
                float(getattr(self, 'dual_traj_resample_gap_trigger', 0.18))
            )
            total_sec_now = max(0.0, float(t_unified[-1]))
            max_points = max(20, int(getattr(self, 'dual_traj_resample_max_points', 260)))
            dt_by_cap = (
                total_sec_now / float(max_points - 1)
                if total_sec_now > 1e-6 and max_points > 1
                else resample_dt_base
            )
            resample_dt = max(resample_dt_base, dt_by_cap)

            if max_gap > resample_gap_trigger and total_sec_now > resample_dt:
                old_times = list(t_unified)
                old_positions = [list(pt.positions) for pt in merged.points]

                def interp_positions(target_t):
                    if target_t <= old_times[0]:
                        return list(old_positions[0])
                    if target_t >= old_times[-1]:
                        return list(old_positions[-1])
                    lo = 0
                    hi = len(old_times) - 1
                    while hi - lo > 1:
                        mid = (lo + hi) // 2
                        if old_times[mid] < target_t:
                            lo = mid
                        else:
                            hi = mid
                    t1 = old_times[lo]
                    t2 = old_times[hi]
                    ratio = (target_t - t1) / max(t2 - t1, 1e-6)
                    return [
                        p1 + ratio * (p2 - p1)
                        for p1, p2 in zip(old_positions[lo], old_positions[hi])
                    ]

                resampled_times = [0.0]
                cursor = resample_dt
                while cursor < total_sec_now - 1e-6:
                    resampled_times.append(cursor)
                    cursor += resample_dt
                if resampled_times[-1] < total_sec_now:
                    resampled_times.append(total_sec_now)

                resampled_points = []
                for t_new in resampled_times:
                    point = JointTrajectoryPoint()
                    point.positions = interp_positions(t_new)
                    set_dur(point, t_new)
                    resampled_points.append(point)

                merged.points = resampled_points
                t_unified = resampled_times
                self.get_logger().info(
                    f"合并轨迹重采样: {len(old_times)}→{len(resampled_times)}点, "
                    f"dt={resample_dt:.3f}s, max_gap={max_gap:.3f}s")
        ...
        try:
            ...
            if not gh.accepted:
                ...
                return False
            exec_timeout = max(30.0, total_sec + 20.0)
    ...
    async def sync_move_arms_coordinated(self, left_pose, right_pose):
        ...
        for i, pt in enumerate(resampled_points):
            ...
            name_to_pos = {str(n): p for n, p in zip(ik_js.name, ik_js.position)}
            missing_joints = [jn for jn in self.right_arm_joints if jn not in name_to_pos]
            if missing_joints:
                self.get_logger().warn(
                    f"【IK】 第 {i} 航点结果缺少右臂关节: {missing_joints}，抛弃轨迹")
                return False
            right_pos = [float(name_to_pos[jn]) for jn in self.right_arm_joints]
